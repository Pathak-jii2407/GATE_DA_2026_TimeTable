...
# -------------------------------------------------------------
def safe_rerun():
    # Use st.rerun if available; fall back to experimental for very old versions
    try:
        st.rerun()
    except Exception:
...
# Set this to True to allow only one total session (even same user/device)
STRICT_SINGLE_SESSION = False  # if True, strictly one session allowed at a time
...
def load_conversation() -> List[Dict[str, Any]]:
    # Load and prune messages to only keep today's entries
...
# -------------------------------------------------------------
# Conversation View (global users + messages for today only)
# -------------------------------------------------------------
